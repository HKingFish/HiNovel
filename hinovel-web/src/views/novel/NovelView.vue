  ...
  <div class="novel-editor-page">
    ...
    <header class="editor-header">
      ...
      <div class="header-right">
        ...
        <el-button type="primary" :icon="Upload" @click="publishChapter">
          发布
        </el-button>
      </div>
    </header>
    ...
    <div class="editor-body">
      ...
      <aside class="editor-sidebar" :class="{ collapsed: isSidebarCollapsed }">
        ...
        <div class="sidebar-header">
          <template v-if="!isSidebarCollapsed">
            ...
            <div class="sidebar-header-actions">
              ...
              <el-button :icon="DArrowLeft" text size="small" circle @click="isSidebarCollapsed = true"/>
            </div>
          </template>
          <template v-else>
              ...
              <template v-if="currentChapterInfo">
                <div class="collapsed-chapter-info">
                  ...
                  <span
                    ...
                    :class="currentChapterInfo.status === 'PUBLISHED' ? 'dot-published' : (currentChapterInfo.needRepublish ? 'dot-need-republish' : 'dot-draft')"
                  ></span>
                  <!-- 章节序号（竖排） -->
                  <span class="collapsed-chapter-number">第{{ currentChapterInfo.chapterNumber }}章</span>
                  <!-- 字数（竖排） -->
                  <span class="collapsed-word-count">{{ formatWordCount(currentChapterInfo.wordCount) }}</span>
                </div>
              </template>
              <!-- 总章节数 -->
        ...
        <div v-show="!isSidebarCollapsed" class="sidebar-content" ref="chapterListRef"
                ...
                <div class="chapter-content">
                  <div class="chapter-header-row">
                    ...
                    <div class="chapter-actions">
                      <el-button :icon="EditPen" text circle size="small" @click.stop="openEditChapterDialog(chapter)" />
                      <el-button :icon="Delete" text circle size="small" @click.stop="confirmDeleteChapter(chapter)" />
                    </div>
                  </div>
                  ...
                  <div class="chapter-status-row">
                    ...
                    >
                      <span class="status-badge" :class="getPublishStatusClass(chapter)">
                        ...
                        <span class="status-text">{{ getPublishStatusLabel(chapter) }}</span>
                      </span>
                    </el-tooltip>
                    <!-- 向量存储状态标志 -->
                    <el-tooltip
                        :content="chapter.vectorStored ? '点击重新同步到向量数据库' : '点击同步到向量数据库'"
                        placement="top"
                        :show-after="300"
                    >
                      <span class="status-badge"
                            :class="[
                              chapter.vectorStored ? 'status-vector-done' : 'status-vector-none',
                              'status-clickable'
                            ]"
                            @click.stop="syncChapterVector(chapter)"
                      >
                        <el-icon :size="11"><Connection/></el-icon>
                        <span class="status-text">{{ chapter.vectorStored ? '已入库' : '未入库' }}</span>
                      </span>
                    </el-tooltip>
                  </div>
          ...
          <div class="content-actions">
            ...
            <el-tooltip content="清除多余空白行" placement="bottom" :show-after="300">
              <el-button
                  size="small"
                  text
                  class="clean-blank-btn"
                  @click="cleanBlankLines"
              >
                <el-icon :size="14">
                  <Delete/>
                </el-icon>
            ...
            <el-button 
              ...
              :loading="isSaving"
              @click="manualSave"
              :class="['save-btn', hasUnsavedChanges ? 'save-btn-unsaved' : 'save-btn-saved', { 'save-btn-pulse': hasUnsavedChanges }]"
            >
      ...
      <!-- 正文面板与功能区之间的拖拽手柄 -->
      <div
          v-show="activeFunctionTab"
          class="panel-divider-handle"
          @mousedown="startResizeFunctionPanel"
      ></div>
                  ...
                  <div class="inline-detail-header">
                    <div class="inline-detail-avatar">
                      <el-avatar
                          ...
                          :style="{ background: selectedCharacter.color || '#6366f1', fontSize: '18px' }"
                      >
                      ...
                      <div class="inline-detail-name">
                        ...
                        <el-tag
                            :type="selectedCharacter.roleType === 'PROTAGONIST' ? 'danger' : selectedCharacter.roleType === 'ANTAGONIST' ? 'warning' : 'info'"
                            size="small"
                        >
                  ...
                  <div class="inline-detail-body">
                    ...
                    <div v-if="selectedCharacterRelations.length > 0" class="inline-relations">
                      ...
                      <div class="inline-relations-list">
                        ...
                        >
                          <el-tag size="small" effect="plain" round>{{ rel.relationType }}</el-tag>
                          <span class="inline-relation-name">
                            {{
                              characterData.find(c => c.id === (rel.characterId === selectedCharacter?.id ? rel.targetId : rel.characterId))?.name || '未知人物'
                            }}
                          </span>
                        </div>
                      </div>
                    ...
                    <!-- 无任何详细信息时的提示 -->
                    <div
                        v-if="!selectedCharacter.gender && !selectedCharacter.age && !selectedCharacter.identity && !selectedCharacter.personality && !selectedCharacter.background && selectedCharacterRelations.length === 0"
              ...
              <template v-else>
                  ...
                  >
                    <div class="version-info">
                      <div class="version-left">
                        ...
                        <!-- 发布标记 -->
                        <el-tag v-if="version.published === 1" size="small" type="success" class="published-tag">
                          已发布
                        </el-tag>
                      </div>
                ...
                <div v-if="previewVersionId" class="inline-version-preview">
                  <div class="preview-header">
                    <span>版本预览</span>
                    <el-button text size="small" @click="previewVersionId = null; previewVersionContent = ''">
                      关闭
                    </el-button>
                  </div>
              ...
              >
                ...
                >
                  <div class="session-option-item">
                    <el-icon v-if="session.favorite === 1" :size="12" color="#f59e0b" class="session-star-icon">
                      <Star/>
                    </el-icon>
            ...
            <div class="tab-panel-body chat-panel-body">
              <div class="chat-messages-wrapper">
                <div class="inline-chat-messages" ref="chatMessagesRef" @scroll="handleChatScroll">
                  ...
                  <div v-else-if="!chatHasMore && chatMessages.length > CHAT_PAGE_SIZE" class="chat-no-more">已加载全部消息</div>
                  <div
                  ...
                  >
                    ...
                    <div class="message-avatar">
                      <el-avatar :size="28"
                                 :style="{ background: msg.role === 'user' ? '#1a73e8' : '#667eea', fontSize: '12px' }">
                        {{ msg.role === 'user' ? '我' : 'AI' }}
                      </el-avatar>
                    </div>
                    <div class="message-content">
                      ...
                      <!-- AI 回复：Markdown 渲染 -->
                      <div v-else class="message-text ai-markdown-content" v-html="renderMarkdown(msg.content)"></div>
                      <!-- 消息操作按钮 -->
                      <div v-if="msg.content" class="message-actions">
                        <el-tooltip v-if="msg.role === 'assistant'" content="复制回答" placement="bottom" :show-after="300">
                          <el-button
                              text
                              size="small"
              ...
              <div v-else class="annotation-list">
                ...
                >
                  ...
                  <div v-if="annotation.status === 'PENDING'" class="annotation-list-actions">
                    <button
                        class="adopt-btn"
                        @click.stop="handleAdoptAnnotation(annotation)"
                    >
                      采纳
                    </button>
                    <button
                        class="ignore-btn"
                        @click.stop="handleResolveAnnotation(annotation.id)"
                    >
                      忽略
                    </button>
                  </div>
              ...
              <div v-show="settingsActiveTab === 'ai-settings'" class="settings-scroll-area">
                ...
                <div class="settings-group">
                  <div class="settings-group-title">
                    <span class="settings-group-dot" style="background: #667eea;"></span>
                    发布后自动操作
                  </div>
                  <div class="settings-switch-list">
                    <div class="settings-switch-item">
                      <div class="setting-info">
                        ...
                        <span class="setting-desc">改写完成后自动调用 AI 审核</span>
                      </div>
                      <el-switch v-model="novelSettingsForm.autoAuditAfterRewrite" :active-value="1"
                                 :inactive-value="0"/>
                    </div>
                    <div class="settings-switch-item">
                      <div class="setting-info">
                        <span class="setting-label">自动生成 AI 大纲</span>
                        <span class="setting-desc">发布后自动总结章节大纲</span>
                      </div>
                      <el-switch v-model="novelSettingsForm.autoOutlineAfterPublish" :active-value="1"
                                 :inactive-value="0"/>
                    </div>
                    <div class="settings-switch-item">
                      <div class="setting-info">
                        <span class="setting-label">自动存储向量</span>
                        <span class="setting-desc">发布后自动存入向量数据库</span>
                      </div>
                      <el-switch v-model="novelSettingsForm.autoVectorAfterPublish" :active-value="1"
                                 :inactive-value="0"/>
                    </div>
                    <div class="settings-switch-item">
                      <div class="setting-info">
                      ...
                      <el-switch v-model="novelSettingsForm.autoSaveContent" :active-value="1"
                                 :inactive-value="0"/>
                    </div>
                  </div>
                </div>
                ...
                <div class="settings-group">
                  <div class="settings-group-title">
                    <span class="settings-group-dot" style="background: #f59e0b;"></span>
                    AI 改写设置
                  </div>
                  ...
                  <div class="settings-number-hint">AI 改写时携带前几章内容（0 表示不携带）</div>
                  <div class="settings-switch-list" style="margin-top: 10px;">
                    <div class="settings-switch-item">
                      <div class="setting-info">
                        <span class="setting-label">改写携带全文大纲</span>
                        <span class="setting-desc">改写时将全文大纲作为参考</span>
                      </div>
                      <el-switch v-model="novelSettingsForm.rewriteIncludeOutline" :active-value="1"
                                 :inactive-value="0"/>
                    </div>
                    <div class="settings-switch-item">
                      <div class="setting-info">
                        <span class="setting-label">审核携带全文大纲</span>
                        <span class="setting-desc">审核时将全文大纲作为参考</span>
                      </div>
                ...
                <div class="settings-group">
                  <div class="settings-group-title">
                    <span class="settings-group-dot" style="background: #10b981;"></span>
                    AI 问答设置
                  </div>
                  <div class="settings-switch-list">
                    <div class="settings-switch-item">
                      <div class="setting-info">
                        <span class="setting-label">问答携带全文大纲</span>
                        <span class="setting-desc">问答时将全文大纲作为参考</span>
                      </div>
                      <el-switch v-model="novelSettingsForm.qaIncludeOutline" :active-value="1"
                                 :inactive-value="0"/>
                    </div>
                  </div>
                  <div class="settings-number-row" style="margin-top: 10px;">
                    <span class="setting-label">历史消息条数</span>
                    <el-input-number
                        v-model="novelSettingsForm.qaContextLength"
                        :min="0"
                        :max="50"
                        :step="1"
                        size="small"
                  ...
                  <div class="settings-number-hint">AI 问答时携带的历史对话条数（0 表示不携带）</div>
                </div>
              </div>
            </div>
    ...
    >
      <div class="ai-outline-content">
        ...
        <el-input
            ...
            :rows="12"
            readonly
            class="ai-outline-textarea"
        />
      </div>
      <template #footer>
        <el-button @click="showAiOutlineDialog = false">仅作参考</el-button>
        <el-button type="primary" @click="handleReplaceOutline">
          替换我的大纲
        </el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog
      ...
      :title="'AI 审核结果 - ' + (auditResult?.overallAssessment === 'pass' ? '通过' : auditResult?.overallAssessment === 'warning' ? '警告' : '失败')"
      width="800px"
      ...
      <div v-if="auditResult" class="audit-result-content">
        <div class="audit-summary">
          <div class="summary-item">
            ...
            <el-tag :type="auditResult.overallAssessment === 'pass' ? 'success' : auditResult.overallAssessment === 'warning' ? 'warning' : 'danger'">
              {{ auditResult.overallAssessment === 'pass' ? '通过' : auditResult.overallAssessment === 'warning' ? '警告' : '失败' }}
            </el-tag>
          </div>
          <div class="summary-item">
          ...
          >
            <div class="issue-header">
              <el-tag :type="issue.severity === 'high' ? 'danger' : issue.severity === 'medium' ? 'warning' : 'info'" size="small">
                {{ issue.issueType === 'consistency' ? '一致性' : issue.issueType === 'logic' ? '逻辑' : issue.issueType === 'character' ? '人物' : '剧情' }}
              </el-tag>
              <el-tag :type="issue.severity === 'high' ? 'danger' : issue.severity === 'medium' ? 'warning' : 'info'" size="small">
        ...
        <div class="audit-actions">
          ...
          <el-button type="primary" @click="regenerateContent" v-if="auditResult.overallAssessment !== 'pass'">
            重新生成
          </el-button>
        </div>
      </div>
  ...
  try {
    ...
    if (draftStr) {
      ...
      if (draft.chapterId == currentChapterId.value && draft.novelId == novelId.value) {
        ...
        const hasOutlineDiff = (draft.chapterOutline && draft.chapterOutline !== chapterOutline.value)
...
const handleResizeOutline = (e: MouseEvent) => {
  ...
  outlinePanelWidth.value = Math.min(
      OUTLINE_PANEL_MAX_WIDTH,
      Math.max(OUTLINE_PANEL_MIN_WIDTH, newWidth)
  )
}
 ...
 */
const handleAcceptRewrite = async (data: {
  annotationId: string
  originalText: string
...
const jumpToChapter = async (targetChapterNumber: number) => {
  // 在已加载列表中查找目标章节
  const existingIndex = chapters.value.findIndex(
  ...
  // 加载完成后查找并滚动
  await nextTick()
  const loadedIndex = chapters.value.findIndex(
      (ch) => ch.chapterNumber === targetChapterNumber
  )
  if (loadedIndex >= 0) {
...
const openCreateChapterDialog = () => {
  ...
  // 默认章节号为当前最大章节号 + 1
  const maxNumber = chapters.value.reduce(
      (max, c) => Math.max(max, c.chapterNumber), 0
  )
...
const addChapter = async () => {
  ...
  const duplicateChapter = chapters.value.find(
      c => c.chapterNumber === newChapterNumber.value
  )
  if (duplicateChapter) {
...
const updateChapter = async () => {
  ...
  const duplicateChapter = chapters.value.find(
      c => c.id !== editingChapter.value.id
          && c.chapterNumber === editingChapter.value.chapterNumber
  )
  if (duplicateChapter) {
    ElMessage.warning(`第 ${editingChapter.value.chapterNumber} 章已存在（${duplicateChapter.title}），请更换章节号`)
    return
  }
  ...
  try {
    ...
    const chapterNumberChanged = originalChapter &&
        editingChapter.value.chapterNumber !== originalChapter.chapterNumber
...
const confirmDeleteChapter = (chapter: Chapter) => {
  ElMessageBox.confirm(
    `确定要删除第${chapter.chapterNumber}章「${chapter.title || '未命名章节'}」吗？`,
    '确认删除',
    {
...
/** 章节相关工具名称 */
const CHAPTER_TOOL_NAMES = [
  'createChapterWithContent', 'updateChapterContent',
  'saveChapterOutline'
]
...
const CHARACTER_TOOL_NAMES = [
  'createCharacter', 'updateCharacterBasicInfo',
  'updateCharacterProfile', 'createCharacterRelation'
]
...
const getCharacterRelations = (characterId: number): NovelCharacterRelation[] => {
  return characterRelationData.value.filter(
...
const startResizeFunctionPanel = (event: MouseEvent) => {
  ...
  const onMouseMove = (moveEvent: MouseEvent) => {
    ...
    const newWidth = Math.min(
        FUNCTION_PANEL_MAX_WIDTH,
        Math.max(FUNCTION_PANEL_MIN_WIDTH, startWidth + deltaX)
    )
...
const hasOutlineActuallyChanged = (): boolean => {
  return chapterOutline.value !== lastSyncedOutline.value.chapterOutline
...
const checkOutlineChanged = (): boolean => {
  return chapterOutline.value !== lastSavedOutline.value.chapterOutline
